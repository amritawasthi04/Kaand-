...
class WorkerService {
  Future<List<Article>> fetchNews({String? category, String? query, String? hl, String? gl}) async {
    ...
    if (query != null && query.trim().isNotEmpty) {
      rssUrl = 'https://news.google.com/rss/search?q=${Uri.encodeComponent(query)}&hl=$hlVal&gl=$glVal&ceid=$glVal:en';
    } else if (category != null && category.toLowerCase() != 'all' && category.toLowerCase() != 'general') {
      rssUrl = 'https://news.google.com/rss/headlines/section/topic/${Uri.encodeComponent(category.toUpperCase())}?hl=$hlVal&gl=$glVal&ceid=$glVal:en';
    } else {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to fetch news directly from Google News: status ${response.statusCode}');
      }
  ...
  List<Article> _parseRSS(String xml) {
    ...
    final linkRegex = RegExp(r'<link[^>]*>([\s\S]*?)</link>', caseSensitive: false);
    final dateRegex = RegExp(r'<pubDate[^>]*>([\s\S]*?)</pubDate>', caseSensitive: false);
    ...
    for (final match in itemRegex.allMatches(xml)) {
      ...
      String source = srcMatch != null ? (srcMatch.group(1) ?? srcMatch.group(2) ?? 'Google News').trim() : 'Google News';
